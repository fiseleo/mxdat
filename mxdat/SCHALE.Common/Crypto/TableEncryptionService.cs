...
namespace SCHALE.Common.Crypto
{
    public static class TableEncryptionService
    {
        ...
        {
            using (var xxhash = XXHash32.Create()) // 确保 `XXHash` 库已正确引用
            {
                xxhash.ComputeHash(Encoding.UTF8.GetBytes(key));
                var mt = new MersenneTwister((int)xxhash.HashUInt32);
                var keyBytes = mt.NextBytes(input.Length); // 修改为输入字符串长度
                var inputBytes = Encoding.UTF8.GetBytes(input);
                Crypto.XOR.Crypt(inputBytes, keyBytes);
                return Encoding.UTF8.GetString(inputBytes); // 将修改后的字节数组转换回字符串
            }
